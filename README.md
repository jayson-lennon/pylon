...
This will result in minified `/output/logo.svg` and `/output/popup.svg` having color `#AABBCC` replaced with `#DDEEFF`.
...
Per-document data can be set for specific documents based on a glob pattern. The context will be available in templates using the identifier given in the closure. However, using the same name as a [builtin context identifier](#context-builtins) is an error and the build will be aborted.
...
| `doc`       | Container for page related information                                            |
| `doc.path`  | On-disk path to the Markdown file for the page                                    |
